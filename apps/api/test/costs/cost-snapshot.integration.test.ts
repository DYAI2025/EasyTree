 ...
 *
 * - `create`: $5 und $7 tauschen (`position.worksiteLabel` <->
 *   `position.employeeLabel`) -> Fall 1 rot. Gemessen faellt schon der
 *   Rundlauf (Lauf 31645195574, Zeile 430); die Spaltenzusicherung kommt gar
 *   nicht mehr dran, weil vitest beim ersten `expect` abbricht.
 * - Denselben Tausch ZUSAETZLICH in `zuPosition` -> der Rundlauf hebt ihn auf
 *   und bleibt gruen, allein die Spaltenzusicherung faellt (Lauf 31645800409,
 *   Zeile 457). Das ist ihre Daseinsberechtigung.
 * - Der Tausch NUR in `zuPosition` -> die Spalten stehen richtig, die Antwort
 *   luegt: allein der Rundlauf faellt (Lauf TODO-LAUF-C). Das ist SEINE
 *   Daseinsberechtigung, denn keine andere Suite prueft diese Abbildung.
 *
