...
|---|---|---|---|
| OQ-001 | Bestätigt der Product Owner Vision und Canvas dieses Feature-Schnitts? | MISSING | Blockiert `READY_FOR_AGILETEAM_PLANNING` |
| OQ-002 | Bestätigen die Developers die Umsetzbarkeit innerhalb des aktiven Sprintfensters? | MISSING | Sprint-Commitment bleibt offen |
| OQ-003 | Wird PR #23 vor Beginn gemergt oder bleibt der Feature-Branch bewusst gestapelt? | ASSUMPTION | Beeinflusst Merge- und Review-Reihenfolge |
...
1. EYT-88 und EYT-91 müssen vor dem schreibenden E2E grün sein.
2. Der Command wird test-first hinter einem Application-Port implementiert.
3. Ohne nachgewiesene Atomarität und Idempotenz wird kein UI-Schreibpfad als abgeschlossen behandelt.
4. Die UI wird erst gegen den realen HTTP-Gateway als fertig bewertet.
5. EYT-62 ist das Featureabschluss-Gate; ein grüner Unit-Test-Satz allein reicht nicht.
6. Publish, Auth oder Mitarbeiterfunktionen werden bei Scope-Einschleppung gestoppt und separat geplant.
...
Die Bestätigung wurde noch nicht erteilt und darf nicht durch den Agenten ersetzt werden.
