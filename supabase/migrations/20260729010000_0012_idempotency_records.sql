...
create or replace function app.lock_employee_planning(p_org_id uuid, p_employee_id uuid)
returns void
language sql
volatile
set search_path = ''
as $$
  ...
  -- bei Uuids entsprechend haeufiger waeren.
  --
  -- Organisation UND Person gehen in denselben Hash, getrennt durch ':'.
  -- Ohne den Mandantenanteil koennten gleiche Personen-Uuids verschiedener
  -- Organisationen einander sperren; ohne das Trennzeichen liessen sich zwei
  -- verschiedene Paare zu derselben Zeichenkette zusammensetzen.
  select pg_advisory_xact_lock(
    hashtextextended(p_org_id::text || ':' || p_employee_id::text, 0)
  );
$$;

comment on function app.lock_employee_planning(uuid, uuid) is
  'EYT-92: serialisiert Pruefen-und-Einfuegen von Einsaetzen je Organisation und Person. Transaktionsgebunden — Freigabe beim commit oder rollback, kein manuelles Unlock. Ersetzt KEINE Constraint: die Entwurfsueberlappung bleibt fachlich erlaubt (Migration 0010), nur der gleichzeitige Doppeleinsatz ueber den Nutzerpfad wird verhindert.';

grant execute on function app.lock_employee_planning(uuid, uuid) to authenticated;
